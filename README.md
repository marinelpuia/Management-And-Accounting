...
This is an inventory and accounting program for clothing, footwear and various items for a store. It can be easily adapted for many types of business.

<h3>The story of this program</h3>
...
- Optional you can then create a shortcut for the jar file on the desktop in this way you don't have to open the folder and click on the jar file every time you need to run the program you just double-click on the shortcut from your desktop and if you want you can change the icon on the shortcut by open the properties of the shortcut and then customize and then add a new icon.
...
- To be more exactly when you enter the mouse in the bar chart area you will see in the chart only the profits of the years as columns only if they have value more than zero as a net profit, when you move the mouse in the table area, the chart will start automatically animated whit the data written already in the XML file by using this line of code barChart.setYValue(Math.random() * 60000 in the YearProfitController.java which means the colons will be randomly animated up to 60000 Leva as a net profit. 
